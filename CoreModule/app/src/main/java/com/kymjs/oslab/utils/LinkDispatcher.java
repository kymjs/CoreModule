...
import com.kymjs.oslab.module.blog.BlogDetailActivity;
import com.kymjs.oslab.module.browser.MobelBrowserActivity;
 ...
 */
public class LinkDispatcher implements ILinkDispatcher {
    /**
     * 分发已知适配手机页面的url,对于未知页面调用#BrowserActivity显示
     */
    public void dispatchUrl(Context cxt, String url) {
        if ((getActionTitle(url, null)) != null) {
            MobelBrowserActivity.goinActivity(cxt, url, getActionTitle(url, ""));
        } else {
            BrowserActivity.toBrowser(cxt, url);
        }
    }
     ...
     */
    @Override
    public void dispatch(WebView view, String url) {
     ...
     */
    @Override
    public String getActionTitle(String url, String defTitle) {
        String title;
        //还有哪些移动页面已经适配的网站呢?
